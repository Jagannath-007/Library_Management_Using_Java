...
public class jdbcConnection {
	...
	public static boolean userRegister(UserInfo _userInfo) {
		...
		try {
			...
			ResultSet myResultSet = myStatement
					.executeQuery("select count(*) from userinfo where userAccount = '" + userAccount + "'");
			myResultSet.next();

			if (myResultSet.getInt("count(*)") == 0) {
				status = true;
				myStatement.executeUpdate("insert into userInfo(userAccount, userPassword) values('" + userAccount
						+ "', '" + userPassword + "')");
	...
	public static boolean userLogin(UserInfo _userInfo) {
		...
		try {
			...
			ResultSet checkLoginStatus = myStatement.executeQuery("select count(*) from userInfo where userAccount = '"
					+ userAccount + "' and userPassword = '" + userPassword + "'");
	...
	public static void addBookItem(BookItem _bookItem) {
		...
		try {
			myStatement = myConnection.createStatement();
			ResultSet myResultSet = myStatement.executeQuery("select count(*) from bookItem where bookName = '"
					+ bookName + "' and bookAuthor = '" + bookAuthor + "'");
			myResultSet.next();

			if (myResultSet.getInt("count(*)") == 0) {
				myStatement.executeUpdate("insert into bookItem(bookName, bookAuthor, numOfBooks, bookStatus) values('"
						+ bookName + "', '" + bookAuthor + "', '" + numOfBooks + "', '" + bookStatus + "')");
				...
				myResultSet = myStatement.executeQuery("select * from bookItem where bookName = '" + bookName
				...
				myStatement.executeUpdate(
						"update bookItem set numOfBooks = " + sumNumOfBooks + " where bookId = " + bookId);
				myStatement.executeUpdate(
						"update bookItem set bookStatus = '" + bookStatus + "' where bookId = " + bookId);
	...
	public static boolean searchBookItem(String _bookName, String _bookAuthor) {
		...
		try {
			myStatement = myConnection.createStatement();
			ResultSet myResultSet = myStatement.executeQuery("select count(*) from bookItem where bookName = '"
					+ bookName + "' or bookAuthor = '" + bookAuthor + "'");
			myResultSet.next();
			...
			if (output == 1) {
				...
				if ((bookAuthor).isEmpty())
					GUI.message = bookName + " not found";
				else if ((bookName).isEmpty())
					GUI.message = bookAuthor + " not found";
				else
					GUI.message = bookName + " by " + bookAuthor + " is not found";
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public static boolean borrowBooks(UserInfo _userInfo, BookItem _bookItem) {
		...
		try {
			...
			myResultSet = myStatement.executeQuery(
					"select * from bookItem where bookName = '" + bookName + "' and bookAuthor = '" + bookAuthor + "'");
			myResultSet.next();
		...
		if ((bookStatus).equals("AVAILABLE")) {
			try {
				myStatement = myConnection.createStatement();
				myResultSet = myStatement.executeQuery("select userId from userInfo where userAccount = '" + userAccount
						+ "' and userPassword = '" + userPassword + "'");
				myResultSet.next();
				userId = myResultSet.getInt("userId");

				myResultSet = myStatement
						.executeQuery("select count(*) from loan where uId = " + userId + " and bId = " + bookId);
				myResultSet.next();
				if (myResultSet.getInt("count(*)") == 0) {
					status = true;
					numOfBooks = numOfBooks - 1;
					myStatement.executeUpdate(
							"update bookItem set numOfBooks = " + numOfBooks + " where bookId = " + bookId);
					if (numOfBooks == 0) {
	...
	public static boolean returnBooks(UserInfo _userInfo, BookItem _bookItem) {
		...
		try {
			myStatement = myConnection.createStatement();
			myResultSet = myStatement.executeQuery("select * from userInfo where userAccount = '" + userAccount
					+ "' and userPassword = '" + userPassword + "'");
			myResultSet.next();
			userId = myResultSet.getInt("userId");
		} catch (SQLException e) {
		...
		if (checkLoanRelationExists > 0) {
			...
			try {
				...
				myStatement = myConnection.createStatement();
				myStatement
						.executeUpdate("update bookItem set numOfBooks = " + numOfBooks + " where bookId = " + bookId);
